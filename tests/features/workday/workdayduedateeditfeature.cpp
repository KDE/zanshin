...
private slots:
    void Setting_a_date_s_due_date_to_today_makes_it_appear_in_the_Workday_page()
    {
        ...
        And(I_add_a_task("Make more test tasks"));
        And(there_is_an_item_in_the_central_list("Make more test tasks"));
        When(I_open_the_item_in_the_editor());
        And(I_change_the_editor_field("due date", "2015-03-10"));
        And(I_display_the_page("Workday"));
        And(I_look_at_the_central_list());
        And(I_list_the_items());
        Then(the_list_contains("Make more test tasks"));
    }
    ...
    void Setting_a_date_s_due_date_to_a_date_in_the_past_makes_it_appear_in_the_Workday_page()
    {
        ZANSHIN_CONTEXT;
        Given(I_display_the_page("Inbox"));
        And(I_add_a_task("Buy potatoes"));
        And(there_is_an_item_in_the_central_list("Buy potatoes"));
        When(I_open_the_item_in_the_editor());
        And(I_change_the_editor_field("due date", "2001-03-10"));
        And(I_display_the_page("Workday"));
        And(I_look_at_the_central_list());
        And(I_list_the_items());
        Then(the_list_contains("Buy potatoes"));
    }
};
