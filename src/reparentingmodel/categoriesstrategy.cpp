...
IdList CategoriesStrategy::getParents(const QModelIndex &sourceChildIndex, const IdList& ignore)
{
    ...
    foreach (const QString &category, categories) {
        ...
        Q_ASSERT(mCategoryMap.contains(category));
        parents << mCategoryMap.value(category);
    }
    foreach(Id i, ignore) {
        parents.removeAll(i);
    }
...
void CategoriesStrategy::createCategoryNode(const QString &categoryPath)
{
    Id id;
    Id parent = mRoot;
    QString categoryName = getCategoryName(categoryPath);
    if (categoryPath.contains(CategoryManager::pathSeparator())) {
        QString parentCategory = categoryPath.left(categoryPath.lastIndexOf(CategoryManager::pathSeparator()));
        if (!mCategoryMap.contains(parentCategory)) {
            CategoryManager::instance().addCategory(parentCategory);
        }
        Q_ASSERT(mCategoryMap.contains(parentCategory));
        parent = mCategoryMap.value(parentCategory);
    }
    ...
    if (!mCategoryMap.contains(categoryPath)) {
        ...
        id = mCategoryMap.value(categoryPath);
        return;
    }
    kDebug() << id << categoryPath;
    createNode(id, IdList() << parent, categoryName);
...
bool CategoriesStrategy::reparentOnParentRemoval(Id child) const
{
    ...
    if (mCollectionMapping.values().contains(child)) {
        return false;
    }
    return true;
}
...
void CategoriesStrategy::onNodeRemoval(const Id& changed)
{
    ...
    if (mCategoryMap.values().contains(changed)) {
        CategoryManager::instance().removeCategory(mCategoryMap.key(changed));
    } else if (mCollectionMapping.values().contains(changed)) {
        mCollectionMapping.remove(mCollectionMapping.key(changed));
    }
}
